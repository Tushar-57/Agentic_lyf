...
export type Goal = {
  ...
  linkedPriorities?: string[]; // Array of priority IDs from Personalization
};
...
export type OnboardingData = {
  ...
  planner: PlannerData;
  preferences?: string[];
  isEditMode?: boolean; // Track if user is editing existing profile
  existingProfile?: OnboardingData; // Store existing profile for comparison
};

export type PreferenceCategory = 'Productivity' | 'Health' | 'Finance' | 'Journal' | 'AI' | 'General';
...
export const getSuggestedGoals = (role: UserRole | null) => {
  switch (role) {
    case 'Student':
