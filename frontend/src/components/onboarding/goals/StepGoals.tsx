...
import { Goal, UserRole, SmartCriteria, Answer } from '../utils/onboardingUtils';
import {
...
interface StepGoalsProps {
  ...
  onBack: () => void;
  userPriorities?: Answer[]; // Pass priorities from Personalization step
}
...
export const GoalForm: React.FC<{
  ...
  onCancel: () => void;
  userRole: UserRole | null;
  userPriorities?: Answer[]; // Add priorities here too
}> = ({ goalForm, setGoalForm, onSubmit, onCancel, userRole, userPriorities }) => {
        ...
        <div className="space-y-8">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              {/* Priority Linking Section */}
              {userPriorities && userPriorities.length > 0 && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Link to Your Priorities
                  </label>
                  <p className="text-xs text-gray-500 mb-3">
                    Select priorities from your personalization that relate to this goal
                  </p>
                  <div className="space-y-2">
                    {userPriorities.map((priority) => (
                      <label
                        key={priority.id}
                        className="flex items-center p-3 border border-gray-200 rounded-xl hover:bg-blue-50 cursor-pointer transition-colors"
                      >
                        <input
                          type="checkbox"
                          checked={goalForm.linkedPriorities?.includes(priority.id) || false}
                          onChange={(e) => {
                            const currentLinked = goalForm.linkedPriorities || [];
                            if (e.target.checked) {
                              setGoalForm({
                                ...goalForm,
                                linkedPriorities: [...currentLinked, priority.id],
                              });
                            } else {
                              setGoalForm({
                                ...goalForm,
                                linkedPriorities: currentLinked.filter((id) => id !== priority.id),
                              });
                            }
                          }}
                        ...
                        />
                        <div className="ml-3">
                          <span className="text-sm font-medium text-gray-900">{priority.answer}</span>
...
const StepGoals: React.FC<StepGoalsProps> = ({ selectedGoals, userRole, onSelect, onUpdateGoals, onBack, userPriorities }) => {
